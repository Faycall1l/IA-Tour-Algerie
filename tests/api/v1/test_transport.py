...
async def sample_wilaya_distances(db: AsyncSession):
    distances = [
        WilayaDistance(
            ...
            driving_distance_km=400, driving_time_minutes=240,
            road_classification="autoroute",
        ),
        WilayaDistance(
            origin_wilaya_id=31, dest_wilaya_id=16,
            driving_distance_km=400, driving_time_minutes=240,
            road_classification="autoroute",
        ),
        WilayaDistance(
            origin_wilaya_id=16, dest_wilaya_id=25,
            driving_distance_km=450, driving_time_minutes=270,
...
async def sample_operators(db: AsyncSession):
    ops = [
        TransportOperator(
            name="SNTF", name_ar="الشركة الوطنية للنقل بالسكك الحديدية",
            mode="train", is_active=True,
            phone="+213 21 63 30 00", website="https://sntf.dz",
        ),
        TransportOperator(
            name="Air Algérie", name_ar="الخطوط الجوية الجزائرية",
            mode="flight", is_active=True,
            phone="+213 21 66 40 00", website="https://airalgerie.dz",
        ),
    ]
    db.add_all(ops)
    await db.commit()
...
async def sample_stations(db: AsyncSession):
    line = TransportLine(
        id=uuid.uuid4(), name="Ligne 1 Train", mode="train",
        operator="SNTF", is_active=True,
    )
    db.add(line)
    await db.flush()
    stations = [
        Station(
            id=uuid.uuid4(), name="Gare d'Alger", station_type="train",
            latitude=36.75, longitude=3.06, wilaya_id=16, operator="SNTF",
        ),
        Station(
            id=uuid.uuid4(), name="Gare d'Oran", station_type="train",
            latitude=35.69, longitude=-0.63, wilaya_id=31, operator="SNTF",
        ),
    ]
    db.add_all(stations)
    await db.commit()
...
async def test_get_route_returns_options(
    ...
    with patch("app.api.v1.endpoints.transport.MultiModalRouter") as MockRouter:
        MockRouter.return_value.get_inter_wilaya_options = AsyncMock(
...
async def test_get_route_fallback_to_flat(
    ...
    with patch("app.api.v1.endpoints.transport.MultiModalRouter") as MockRouter:
        MockRouter.return_value.get_inter_wilaya_options = AsyncMock(return_value=[])
        resp = await client.get("/api/v1/transport/routes/16/31")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_list_stations(
    ...
    mock_routing.list_stations = AsyncMock(return_value=[
        {"id": str(s.id), "name": s.name, "station_type": s.station_type, "wilaya_id": s.wilaya_id}
        for s in stations
    ])
...
async def test_list_lines(
    ...
    from app.main import app
    mock_routing = app.state.transit_routing
    mock_routing.list_lines = AsyncMock(return_value=[
        {"id": str(sample_stations[1].id), "name": "Ligne 1 Train", "mode": "train", "operator": "SNTF"}
    ])
...
async def test_list_lines_filter_mode(
    ...
    mock_routing = app.state.transit_routing
    mock_routing.list_lines = AsyncMock(return_value=[
        {"id": str(sample_stations[1].id), "name": "Ligne 1 Train", "mode": "train"}
    ])
...
async def test_plan_route_no_route(client: AsyncClient):
    ...
    resp = await client.get(
        ...
        params={
            "from_lat": 36.75, "from_lng": 3.06,
            "to_lat": 35.69, "to_lng": -0.63,
        },
    )
    assert resp.status_code == 200
    data = resp.json()
