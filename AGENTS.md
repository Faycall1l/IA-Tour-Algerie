...
- **POI photo enrichment** (post-reseed restore + Aug 2026 expansion): **8,898 POIs** have real MinIO-hosted photos (16.9%), 43,787 have category placeholders:
  - **ATHAR MinIO** on `127.0.0.1:19000` (isolated from meddata's 9000), data dir `minio_data/` (gitignored, ~2.2GB, **2,262 objects**), compose base file ports updated
  - Photo passes: `enrich_photos_spatial.py` (Wikidata proximity 500m, **+3,870**), `enrich_photos_commons_category.py` (Commons GPS categories 200m, +62 — DB update was a stub, fixed), `enrich_photos_remaining.py` (~650 named POIs, +227), `enrich_photos_more2.py` (OSM wikidata + SPARQL, exhausted — 0)
  ...
  - **0 Wikimedia URLs remain in DB**; all 8,898 photo-bearing POIs have MinIO `photo_url` primary
  - All 43,787 remaining POIs have category placeholders (`enrich_placeholders.py`)
...
## Next Steps
1. ~~⬜ **Migrate Wikimedia photos to MinIO**~~ — **DONE**: 8,898 POIs migrated to MinIO; **0 Wikimedia URLs remain** in DB (Aug 2026 full pass: spatial +3,870, commons +62, remaining +227, more2 exhausted)
2. **⬅️ More photos for remaining historical/cultural POIs**: 8,898 POIs have real MinIO photos; 43,787 have generated placeholders — Wikidata/Commons name+spatial matching is exhausted; next lever is category-based bulk downloads (e.g., Wikimedia Commons category walks) or user-generated content
